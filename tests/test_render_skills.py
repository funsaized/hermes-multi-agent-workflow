...
class SkillMaterializationTests(unittest.TestCase):
    def test_renders_source_scout_and_orchestrator_to_profile_local_staging(self):
        with tempfile.TemporaryDirectory(prefix="skill render ") as tmp:
            ...
            self.assertIn("python scout_actions.py --config", text)
            self.assertIn("web --report-file", text)
            self.assertIn("Never choose a vault path yourself", text)
            self.assertIn("Pipeline contract: `test-pipeline`", text)
            ...
            self.assertIn("python intake_actions.py --config", orchestrator_text)
            self.assertIn("plan --intake", orchestrator_text)
            self.assertIn("--scores <report>.scores.json", orchestrator_text)
            self.assertIn("python pre_gate_actions.py --config", orchestrator_text)
            self.assertIn('--classification "<value>"', orchestrator_text)
            self.assertIn("approve     test-pipeline:<slug>", orchestrator_text)
            self.assertIn("Never create prep or proposal cards yourself", orchestrator_text)
            self.assertIn("`delivery_actions.py deliver`", orchestrator_text)
            self.assertIn("send-proposal <slug>", orchestrator_text)
            self.assertIn("Never send a proposal with raw", orchestrator_text)
            self.assertIn("discord:briefs via `hermes send` as a single attachment message", orchestrator_text)
            self.assertIn("release barrier, not a second orchestrator pass", orchestrator_text)
            self.assertIn("Never auto-approve", orchestrator_text)
            self.assertIn("block the task with the JSON error", orchestrator_text)
